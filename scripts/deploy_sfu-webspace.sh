...
ssh -f sfu "chmod 777 ${PROJECT_DIR} && rm -rf ${PROJECT_DIR}"

echo "Creating directory '${PROJECT_DIR}' for the project files..."
ssh -f sfu "mkdir -p ${PROJECT_DIR}"
...
ssh -f sfu "chmod 755 ${PACMACRO_DIR} ${PROJECT_DIR}"
ssh -f sfu "chmod 644 ${PROJECT_DIR}/*.* ${PROJECT_DIR}/assets/**/*.*"
